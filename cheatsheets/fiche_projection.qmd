...
![Projection orthogonale de Y sur le sous-espace M(X)](regressiongraph.png) *(Source : Polycopié de cours, Arnaud Guyader)* \* $\hat{Y}$ et $\hat{\beta}$ : Puisque $\hat{Y}$ est dans $\mathcal{M}(X)$, il s'écrit $\hat{Y} = X\hat{\beta}$. Le $\hat{\beta}$ qui permet ça est exactement l'estimateur MCO.
