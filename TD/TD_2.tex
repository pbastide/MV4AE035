...
\begin{document}
    ...
    \end{align*}
			...
			\begin{enumerate}
				...
				= \1^T \hat \varepsilon = \langle \1, \hat \varepsilon \rangle
				= \langle \1, P^{X^\bot} y \rangle
				= 0.
				\]
				Donc :
				\[
				\1^T \hat \varepsilon = \1^T (y - \hat y) = 0 \quad \text{i.e.} \quad \bar y = \frac{1}{n} \1^T\hat y
				\]
				et
				\[
...
Un modèle plus gros a toujours un $R^2$ plus grand.
	...
	d'effectifs $n_{1}, \ldots, n_{I}$, et tels que l'intersection de deux groupes distincts soit vide : 
	\[ C_i \cap C_j = \varnothing \quad   \text{pour tout } i \neq j. \]
	\[
